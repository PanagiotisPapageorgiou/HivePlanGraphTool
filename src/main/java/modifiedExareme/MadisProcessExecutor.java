...
public class MadisProcessExecutor {
    ...
    public MadisExecutorResult exec(ExecuteQueryState state) throws RemoteException {
        try {
            ...
            //Get InputTableViews to determine whether tables are from Hive or not
            List<TableView> inputTableViews = state.getOperator().getQuery().getInputTables();
            TableView outputTableView = state.getOperator().getQuery().getOutputTable();
            List<String> filesToDeleteOnExit = new LinkedList<>();

            // Attach the input tables.
            StringBuilder attachedDBs = new StringBuilder();
            for (String input : state.getOperator().getInputTables()) { //For Every InputTable of this Operator

                //Find whether table is from Hive or not
                boolean tableExists = false;
                boolean isHiveTable = false;
                String hiveTableLocation = "";
                String hiveCreateTableQuery = "";
               List<String> hiveSecondaryQueries = new LinkedList<>();

                for(TableView inputView : inputTableViews){
                    if(inputView.getTable().getName().equals(input)){
                        tableExists = true;
                        isHiveTable = inputView.getIsHiveTable(); //Handle Hive tables differently
                ...
                if(tableExists == false){
                    if(outputTableView.getName().equals(input)){
                        log.error("Final Table: "+input+" equals is input...(look into it)");
                    }
                    else {
                        log.error("Error occured while searching for TableName: " + input + " through the TableViews...");
                        throw new RemoteException();
                    }
                }

                List<Integer> parts = state.getOperator().getInputPartitions(input); //Get all Partitions of this InputTable

                if(isHiveTable == false) { //Table is Exareme table (.db)
                    log.info("Table: "+input+" is a normal Exareme Table...");
                    ...
                    for (int part : parts) {
                        ...
                        if (local == true) {
                            ...
                            try {
                                ...
                                //log.info("Will attempt to fetch with src: "+loc+" and dest: "+loc);
                                log.error(
                ...
                else{ //Table is a Hive Table (.dat) and needs to be converted first
                    log.info("Table: "+input+" is a Hive Table! Need to fetch and treat this case seperately!");
                    //log.info("Currently Equipped for One Table and No Partitions!");

                    for (int part : parts) {
                        String trueHiveLoc = null;
                        String loc = null;
                        String dbName = null;
                        boolean local = false;
                        HashMap<Integer, Integer> partCount = state.getInputPartitions(input);
                        if (partCount == null) { // Local
                            log.info("Hive Table - PartCount == null / Local!");

                            loc = "/tmp/adp" + "/" + input + DBConstants.DB_SEPERATOR + part + ".db";
                            //trueHiveLoc = absoluteDBDir + "/" + hiveTableLocation;
                            trueHiveLoc = absoluteDBDir + "/" + input + DBConstants.DB_SEPERATOR + part + ".db";
                            dbName = input + part;
                            local = true;
                        } else {
                            Integer count = partCount.get(part);
                            if (count == null) { // Local
                                log.info("Hive Table - count == null / Local!");
                                loc = "/tmp/adp" + "/" + input + DBConstants.DB_SEPERATOR + part
                                        + ".db";
                                //trueHiveLoc = absoluteDBDir + "/" + hiveTableLocation;
                                trueHiveLoc = absoluteDBDir + "/" + input + DBConstants.DB_SEPERATOR + part + ".db";
                                dbName = input + part;
                                ...
                                log.info("Hive Table - PartCount != null / remote!");
                                HashMap<Integer, Integer> current = currentCount.get(input);
                                if (current == null) {
                                    current = new HashMap<Integer, Integer>();
                                    currentCount.put(input, current);
                                ...
                                //trueHiveLoc = absoluteDBDir + "/" + hiveTableLocation;
                                trueHiveLoc = absoluteDBDir + "/" + input +
                                        DBConstants.DB_SUBPART_SEPERATOR + part +
                                        DBConstants.DB_SEPERATOR + c + ".db";
                        ...
                        // Attach only local databases
                        if (local == true) {
                            log.info("Hive Table - Local == TRUE / Fetch Case!");

                                log.info("Will try to fetch from Location: "+trueHiveLoc);
                                try {
                                    //String localPath = "/tmp/adp/".concat(hiveTableLocation.replace("/", "_"));
                                    //hiveCreateTableQuery = hiveCreateTableQuery.concat("from (file '"+localPath+"' delimiter:| fast:1) ");

                                    //filesToDeleteOnExit.add(localPath);
                                    filesToDeleteOnExit.add(loc);

                                    //storageClient.fetch(trueHiveLoc, localPath);
                                    storageClient.fetch(trueHiveLoc, loc);
                                } catch (ArmStorageClientException e) {
                                    //log.info("Will attempt to fetch with src: "+loc+" and dest: "+loc);
                                    log.error(
                                            "Error occurred while storage client try to fetch partitions!");
                                ...
                                log.info("Fetched Succesfully: "+trueHiveLoc);
                            ...
                            // Database name
                            log.info("Hive Table - Local == FALSE / Non fetch Case!");
                            ...
                            log.info("Since this .db is a Hive Table we must fetch it anyway!");
                            ...
                            try {
                                ...
                                log.info("Fetched Successfully: " + trueHiveLoc);

                                System.out.println("Now we have to create a proper .db file from this .dat");
...
//                        clonefile.createNewFile();
            ...
            if(filesToDeleteOnExit.size() > 0){
                for(int j = 0; j < filesToDeleteOnExit.size(); j++) {
                    File someFile = new File(filesToDeleteOnExit.get(j));
                    if(someFile.exists()){
                        someFile.delete();
                    }
                }
    ...
    private static void execAndPrintResults(String query, MadisProcess proc, Logger log) throws Exception {
