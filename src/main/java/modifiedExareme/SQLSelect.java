...
public class SQLSelect extends SQLQuery {
    ...
    public void print(){
        ...
        System.out.println("\t\t\tisTemporary: "+isTemporary);
        System.out.println("\t\t\tisScript: "+isScript);
        System.out.println("\t\t\tinputDataPattern: "+inputDataPattern.toString());
        System.out.println("\t\t\toutputDataPattern: "+outputDataPattern.toString());
        System.out.println("\t\t\tnumberOfOutputPartitions: "+numberOfOutputPartitions);

        System.out.println("\t\t\tSQL Query Extra info...");
        this.printSQLQuery();

        if(partitionColumns != null){
            System.out.println("\t\t\tpartitionColumns: ");
            for(String s : partitionColumns){
                System.out.println("\t\t\t\tPartitionColumn: "+s);
            }
        }
        else{
            System.out.println("\t\t\tpartitionColumns: is NULL");
        }
        if(usingTBLs != null){
            System.out.println("\t\t\tusingTBLs: ");
            for(String s : usingTBLs){
                System.out.println("\t\t\t\tUsingTBL: "+s);
            }
        }
        else{
            System.out.println("\t\t\tusingTBLs: is NULL");
        }
    }
